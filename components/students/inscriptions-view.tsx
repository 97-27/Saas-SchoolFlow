...
export function InscriptionsView({
  ...
  const autoGeneratedMatricule = useMemo(() => {
    if (currentSelectedStudent) {
      return currentSelectedStudent.matricule || currentSelectedStudent.studentNumber;
    }
  ...
  const currentMatricule = useMemo(() => {
    if (customMatricule.trim()) {
      return customMatricule.trim();
    }
    return autoGeneratedMatricule;
  }, [customMatricule, autoGeneratedMatricule]);
  ...
  const generateOfficialReceiptCanvas = async (
    ...
    drawRow("Frais d'inscription (Droits & Dossier)", formatFCFA(registrationFee));
        ...
        <div className="relative z-10 border-2 border-slate-300 rounded-xl overflow-hidden text-xs sm:text-sm">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 font-sans">
              {/* Ligne 1 : Frais d'inscription (Droits de dossier) */}
              <tr>
                <td className="text-slate-800 font-bold py-2 px-3 flex items-center justify-between">
                  <span>Frais d&apos;inscription (Droits & Dossier)</span>
              ...
              {discountAmount > 0 && (
                <tr className="bg-amber-50/50 text-amber-950">
                  <td className="font-bold flex items-center gap-1 py-2 px-3">
                    ...
                    <span>Réduction / Bourse accordée</span>
                  </td>
                  <td className="text-right font-black font-mono py-2 px-3 text-amber-800">
              ...
              {paidAmount > 0 && (
                <tr className="bg-emerald-50/30 text-emerald-950">
                  <td className="font-bold py-2 px-3 flex items-center gap-1">
                    <Check className="w-3.5 h-3.5 text-emerald-600" />
                    <span>Versements Scolarité Encaissés</span>
                  </td>
                  <td className="text-right font-black font-mono py-2 px-3 text-emerald-800">
              ...
              {remainingAmount > 0 ? (
                <tr className="text-rose-800 bg-rose-50 font-black">
                  <td className="py-2 px-3">Reste à Payer Scolarité (Solde dû)</td>
                ...
                <tr className="text-emerald-800 bg-emerald-50 font-black">
                  <td className="py-2 px-3">Reste à Payer Scolarité</td>
                  <td className="text-right font-mono py-2 px-3">
            ...
            <div className="p-3 bg-slate-50/90 rounded-2xl border border-slate-200/90 space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  <span>Matricule Officiel de l&apos;élève *</span>
                </label>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-500 font-mono">
                    Ressort : <strong className="text-slate-900">{currentMatricule}</strong>
                  </span>
                  <button
                    type="button"
                    onClick={() => {
              ...
              <input
                ...
                placeholder={`Saisissez le matricule officiel (ou laisser vide pour ${currentMatricule})`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
              ...
              <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  ...
                  >
                    <span>{gender === 'female' ? '🌟 Nouvelle' : '🌟 Nouveau'}</span>
                  </button>
                  ...
                  >
                    <span>{gender === 'female' ? '🔄 Ancienne' : '🔄 Ancien'}</span>
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200 print:hidden">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 sm:p-7 space-y-4 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-2.5 p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-slate-200/70">
                <span className="text-slate-500">Frais d&apos;inscription (Dossier) :</span>
